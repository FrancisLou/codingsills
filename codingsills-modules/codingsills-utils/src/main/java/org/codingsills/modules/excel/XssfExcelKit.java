...
/**
 * 类功能描述
 * XssfExcelHelper.java
 *
 * @date 2016年1月21日
 * 
 * @author Saber
 */
public class XssfExcelKit extends AbstractExcelKit {

    private static XssfExcelKit instance = null; // 单例对象
     ...
     *            文件对象
     */
    private XssfExcelKit(File file){
        super();
        this.file = file;
    }
     ...
     * @return 返回初始化后的单例对象
     */
    public static XssfExcelKit getInstance(File file){
        if(instance == null){
            // 当单例对象为null时进入同步代码块
            synchronized(XssfExcelKit.class){
                // 再次判断单例对象是否为null，防止多线程访问时多次生成对象
                if(instance == null){
                    instance = new XssfExcelKit(file);
                }
            }
        }else{
            // 如果操作的文件对象不同，则重置文件对象
     ...
     * @return 返回初始化后的单例对象
     */
    public static XssfExcelKit getInstance(String filePath){
