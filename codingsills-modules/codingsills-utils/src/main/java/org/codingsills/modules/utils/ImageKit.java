...
public class ImageKit {
     ...
     * @param imageFile
     */
    public ImageKit(File imageFile){
        try{
     ...
     * @param imageFilePath
     */
    public ImageKit(String imageFilePath){
    ...
    public void resize(int newWidth, int newHeight){
        ...
        for(int y = 0; y < oldHeight && newY < newHeight; y++){
            if(isNewImageHeightSmaller && heightCounter > 1){ // new image suppose to be smaller -
                                                              // skip row
                heightCounter -= 1;
                ...
                for(int x = 0; x < oldWidth && newX < newWidth; x++){
                    if(isNewImageWidthSmaller && widthCounter > 1){ // new image suppose to be
                                                                    // smaller - skip column
                        widthCounter -= 1;
    ...
    /**
     * 结合俩张图片
     * jump = 2 表示 第二个像素被新图像取代
     * This makes the 2 images equal in presence. 如果jump=3,第三个像素被新图像取代
     * 
    ...
    public void combineWithPicture(ImageKit image2){
        combineWithPicture(image2.getAsBufferedImage(), 2, null);
    }

    public void combineWithPicture(ImageKit image2, int jump){
        combineWithPicture(image2.getAsBufferedImage(), jump, null);
    }

    public void combineWithPicture(ImageKit image2, Color ignoreColor){
        combineWithPicture(image2.getAsBufferedImage(), 2, ignoreColor);
    }

    public void combineWithPicture(ImageKit image2, int jump, Color ignoreColor){
        combineWithPicture(image2.getAsBufferedImage(), jump, ignoreColor);
    }

    /**
     * 合并两张图片
     * jump = 2 表示每隔两个像素将被新图片取代一个像素
    ...
    public static void main(String[] args){
         ...
         */
        /*
         * Image image = new Image("c:/pics/israel_flag.gif"); Image image2 = new
         * Image("c:/pics/palestine_flag.gif"); //image.resize(50); //image2.resize(50);
         * //image.affineTransform(0, 0.3); //image2.affineTransform(0, -0.3);
         * image.combineWithPicture(image2); image.saveAs("c:/pics/affineTransformAndCombine2.jpg");
         */
        /*
         * Image image = new Image("c:/pics/p1.jpg"); image.resize(50); image.affineTransform(0.0,
         * 0.5); image.saveAs("c:/pics/affineTransform.jpg");
         */
        /*
         ...
         * image2.saveAs("c:/pics/combineWithPictureWithoutBackground.jpg"); /* image.resize(5);
         * image.multiply(20, 20); image.combineWithPicture("c:/p2.jpg");
         * //image.addColorToImage(Color.yellow, 3); //image.addColorToImage(Color.red, 5);
         * //image.combineWithPicture("c:/p2.jpg",3);
         * 
         */
        ImageKit image = new ImageKit("c:/pics/p1.jpg");
