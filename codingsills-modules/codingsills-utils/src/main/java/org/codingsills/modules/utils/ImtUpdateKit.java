...
/**
 * incremental update utils
 * 增量更新工具类，用于增量更新已发布项目时打包
 * ImtUpdateUtils.java
 *
 * @date 2016年4月6日
 * 
 * @author Saber
 */
public final class ImtUpdateKit {

    private static final String PROJECT_PATH = System.getProperty("user.dir");

    private static final String PROJECT_NAME = System.getProperty("user.dir")
            .substring(System.getProperty("user.dir").lastIndexOf(File.separator));
     ...
     *  list.add("SimplexSocket.java"); 
     *  list.add("busiFunc.jsp");
    ...
    public static void createFinalDirectory(List<String> fileNames, String yourDestination){
        ...
        try{
            pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream(update, false), "GBK"),
                    true);
            pw.println("需求:");
            for(int i = 0; i < 10; i++){
                pw.println();
            ...
            for(String fileName : myFileNames){
                ...
                if(fileName.endsWith(".java")){
                    ...
                    String classFileNameForJava =
                            javaFileName.substring(0, javaFileName.lastIndexOf(".")) + ".class";
                    pathFile = finalFiles.get(classFileNameForJava);
                }
                String reallyPath = getReallyPath(yourDestination, pathFile, isJavaFile);
                if(!reallyPath.endsWith(".class")) pw.println("\t\t\t\t\t\t"
                        + file.getCanonicalPath().substring(1 + file.getCanonicalPath().lastIndexOf(
                                PROJECT_PATH.substring(PROJECT_PATH.lastIndexOf(File.separator)))));
     ...
     * @throws IOException
     */
    private static String getReallyPath(String yourDestination, File file, boolean isJavaFile)
            throws IOException{
        ...
        try{
            ...
            for(File file : files){
                ...
                if(map.get(fileName) == null
                        || map.get(fileName).lastModified() < file.lastModified()){
                    map.put(fileName, file);
                    if(fileName.endsWith(".java")){
                        // 将同路径下的class文件加入到map中.
                        for(File classFile : classFiles){
                            String classFileName = classFile.getCanonicalPath();
                            String myClassFileName = classFileName.substring(
                                    classFileName.indexOf("classes" + File.separator) + 7,
                                    classFileName.length() - 6) + ".java";
                            if(file.getCanonicalPath().endsWith(myClassFileName)){
                                ...
                                for(File innerClassFile : innerClassFiles){
                                    ...
                                    String classFileNameWithOutExtend = classFileName.substring(0,
                                            classFileName.lastIndexOf("."));
                                    if(innerClassFilePath
                                            .startsWith(classFileNameWithOutExtend + "$"))
